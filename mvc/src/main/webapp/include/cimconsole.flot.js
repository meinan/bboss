	...
	/**
	  ...
	  @param target_ 鏄剧ず鍥惧舰鐨刣iv瀵硅薄锛坖Query瀵硅薄锛?	  @param data_ json鏍囧噯鏍煎紡鐨勫崟缁勬垨澶氱粍鏁版嵁
	  	鍗曠粍鏁版嵁鏍煎紡涓句緥锛歿"label":"CPU浣跨敤淇℃伅","data":[[1270621129000,18.78121878121878],[1270621437000,0]]}
	  @param threshold_ y杞村垎鐣岀嚎鐨勯榾鍊?	  @param ymax_ y杞寸殑鏈€澶у€?	  @param unit_ y杞存暟鎹殑鍗曚綅
	 */	 
	 function plotTrend(target_, data_, threshold_, ymax_, unit_){
         ...
         //濡傛灉鎸囧畾浜唝杞存渶澶у€硷紝鍒欒繘琛岃缃?         if (ymax_){
	 ...
	 /**鏄剧ず鏁版嵁鐐圭殑鎻愮ず淇℃伅
	 	@param x x鍧愭爣鍊?	 	@param y y鍧愭爣鍊?	 	@param contents 鎻愮ず淇℃伅鍐呭
	 */
	        ...
	        if (true) {
	            if (item) {
	                if (toolTipPreviousPoint != item.datapoint) {
	                    ...
	                    dateTime.setTime(x);	
	                    //灏唋ong鍨嬬殑鏃堕棿杞崲鎴愭湰鍦版椂闂存牸寮?						var dateTimeStr = dateTime.toLocaleString();
