...
function blurryQuary(queryTextname,queryBtn,nextBtn,root_id){
	...
	for(var i = 0; i < a.length; i++){
		...
		if(a[i].name != ""){
			//alert(a[i].name);
			if($(a[i]).parent()){
				$(a[i]).parent().parent().children()[0].style.backgroundColor="";
			}
			if($(a[i]).text().indexOf(blurryValue) != -1){
				//alert(a[i].innerText);
				...
				var tree_node_ = $("a[name='"+eleArray_queryTreenode[count]+"']");
...
//		var tree_node_ = document.all(eleArray_queryTreenode[0])
		...
		var id_ = $(node).attr("id");	
		if(id_ == root_id){
				$(tree_node_).parent().children()[0].css("backgroundColor","cornflowerblue");
				$(tree_node_).parent().focus();
				$("#"+queryBtn).hide();
	   			$("#"+nextBtn).show();
		 		next_queryTreenode ++;
	 		}
 		...
 		for( var i = parentEle.length - 1; i >=0 ; i --){
 			//alert("i = " + i)
 			//if(Element.visible( parentEle[i])){//return $(element).style.display != 'none';
 			//alert($(parentEle[i]).css('display'));
 			if($(parentEle[i]).css('display') != null && $(parentEle[i]).css('display')!="none"){
	   		
	   		}else{
	   		//alert(parentEle[i].substring(11,parentEle[i]));
//	   		   document.all('icon_'+parentEle[i].substring(11,parentEle[i].length)).click();
	   			$('#icon_'+parentEle[i].substring(11,parentEle[i].length)).click();
	   		}

	   		/**判断是否到达最后一个**/ 
	   		if( i == 0 ){
	   			$(tree_node_).parent().children()[0].css("backgroundColor","cornflowerblue");
	   			$(tree_node_).parent().parentElement.focus();
	   			$("#"+queryBtn).hide();
	   			$("#"+nextBtn).show();
		 		next_queryTreenode ++;
	   		}
...
{
	var treenode = eleArray_queryTreenode[nextcount];
	var tree_node_ = $("a[name='"+treenode+"']");
...
/*
	...
	nextBtn		查询下一个按钮名称
*/			
function enterKeydowngo(queryTextname,queryBtn,nextBtn,root_id,event){ 
