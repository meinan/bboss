...
var DataGrid = function(id, _params) {
	...
	this.getQueryParams = function(){
		DataGrid.log("rejectChanges()");
		var queryParams = $(this.id).datagrid('options').queryParams;
		DataGrid.log("rejectChanges().queryParams="+queryParams);
		return queryParams;
	}
	/*方法代理*/
	this.run = function(){
...
DataGrid.log = function(msg,level) {
	...
	if (DataGrid.debug) { 
		...
		if (typeof(msg) == "string") {
			...
			DataGrid.logs += "<"+(new Date()).toTimeString()+"><br>"+html;		
		} else {
...
DataGrid.showLogs = function() {
	if (true != DataGrid.debug) {
		DataGrid.logs = "<font color='red'>需要设置为debug=true后才可记录调试信息！</font>";
	}
	$.messager.alert('调试信息',DataGrid.logs);
	DataGrid.clearLogs();
}
/*清空日志*/
