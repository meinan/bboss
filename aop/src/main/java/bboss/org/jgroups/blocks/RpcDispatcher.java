 ...
 * group members and optionally wait for the return value(s). An application
 * will typically create a channel and layer the RpcDispatcher building block on
 * top of it, which allows it to dispatch remote methods (client role) and at
 * the same time be called by other members (server role). This class is derived
 * from MessageDispatcher. Is the equivalent of RpcProtocol on the application
...
public class RpcDispatcher extends MessageDispatcher implements ChannelListener {
	...
	/**
	 * Marshaller to marshall requests at the caller and unmarshal requests at
	 * the receiver(s)
	 */
	protected Marshaller2 req_marshaller = null;

	/**
	 * Marshaller to marshal responses at the receiver(s) and unmarshal
	 * responses at the caller
	 */
	...
	public RpcDispatcher(Channel channel, MessageListener l,
			MembershipListener l2, Object server_obj) {
		super(channel, l, l2);
	...
	@Deprecated
	public RpcDispatcher(Channel channel, MessageListener l,
			MembershipListener l2, Object server_obj, boolean deadlock_detection) {
		super(channel, l, l2);
		channel.addChannelListener(this);
		this.server_obj = server_obj;
	}
	...
	public RspList callRemoteMethods(Vector<Address> dests, String method_name,
			Object[] args, Class[] types, int mode, long timeout,
			boolean use_anycasting, RspFilter filter) {
		MethodCall method_call = new MethodCall(method_name, args, types);
		return callRemoteMethods(dests, method_call, new RequestOptions(mode,
				timeout, use_anycasting, filter, (byte) 0));
	}
	...
	public RspList callRemoteMethods(Vector<Address> dests, String method_name,
		...
		return callRemoteMethods(dests, method_call, new RequestOptions(mode,
				timeout, use_anycasting, null, (byte) 0));
	}
	...
	@Deprecated
	public RspList callRemoteMethods(Vector<Address> dests,
			MethodCall method_call, int mode, long timeout) {
		return callRemoteMethods(dests, method_call, new RequestOptions()
				.setMode(mode).setTimeout(timeout));
	}

	/**
	 * Invokes a method in all members contained in dests (or all members if
	 * dests is null).
	 * 
	 * @param dests
	 *            A list of addresses. If null, the method will be invoked on
	 *            all cluster members
	 * @param method_call
	 *            The method (plus args) to be invoked
	 * @param options
	 *            A collection of call options, e.g. sync versus async, timeout
	 *            etc
	 * @return RspList A list of return values and flags (suspected, not
	 *         received) per member
	 * @since 2.9
	 */
	public RspList callRemoteMethods(Collection<Address> dests,
			MethodCall method_call, RequestOptions options) {
		if (dests != null && dests.isEmpty()) { // don't send if dest list is
												// empty
			if (log.isTraceEnabled())
				log.trace(new StringBuilder("destination list of ").append(
						method_call.getName()).append(
						"() is empty: no need to send message"));
			return RspList.EMPTY_RSP_LIST;
		}

		if (log.isTraceEnabled())
			log.trace(new StringBuilder("dests=").append(dests).append(
					", method_call=").append(method_call).append(", options=")
					.append(options));

		Object buf;
		try {
			buf = req_marshaller != null ? req_marshaller
					.objectToBuffer(method_call) : Util
					.objectToByteBuffer(method_call);
		} catch (Exception e) {
			// if(log.isErrorEnabled()) log.error("exception", e);
			// we will change this in 3.0 to add the exception to the signature
			// (see http://jira.jboss.com/jira/browse/JGRP-193). The reason for
			// a RTE is that we cannot change the
			// signature in 2.3, otherwise 2.3 would be *not* API compatible to
			// prev releases
			throw new RuntimeException("failure to marshal argument(s)", e);
		}

		Message msg = new Message();
		if(method_call.getCallContext() != null && 
				method_call.getCallContext().getHeaders() != null 
				&& method_call.getCallContext().getHeaders().size() > 0)
		{
			msg.setBbossHeaders(method_call.getCallContext().getHeaders());
		}
		// UPDATE ENCRYPT request MESSAGE BODY
		...
		if (buf instanceof Buffer) {
			...
			if(encrypt)
			{
				byte[] temp = null;
				try {
					temp = RequestCorrelator.encrypt((Buffer) buf);
					msg.setBuffer(temp);
				} catch (Exception e) {
			...
			if(encrypt)
			{
				byte[] temp = null;
				try {
					temp = RequestCorrelator.encrypt((byte[]) buf);
					msg.setBuffer(temp);
				} catch (Exception e) {
					msg.setEncrypt(false);
					msg.setBuffer((Buffer) buf);
...
//			msg.setBuffer((byte[]) buf);
		...
		if (log.isTraceEnabled())
			log.trace("responses: " + retval);
		return retval;
	}

	@Deprecated
	public NotifyingFuture<RspList> callRemoteMethodsWithFuture(
			Vector<Address> dests, MethodCall method_call, int mode,
			long timeout, boolean use_anycasting, boolean oob, RspFilter filter) {
		RequestOptions options = new RequestOptions(mode, timeout,
				use_anycasting, filter);
		if (oob)
	...
	@Deprecated
	public NotifyingFuture<RspList> callRemoteMethodsWithFuture(
			Vector<Address> dests, MethodCall method_call) {
		return callRemoteMethodsWithFuture(dests, method_call,
				new RequestOptions());
	}

	public NotifyingFuture<RspList> callRemoteMethodsWithFuture(
			Collection<Address> dests, MethodCall method_call,
			RequestOptions options) {
		if (dests != null && dests.isEmpty()) { // don't send if dest list is
												// empty
			if (log.isTraceEnabled())
...
//			msg.setBuffer((byte[]) buf);
		...
		NotifyingFuture<RspList> retval = super.castMessageWithFuture(dests,
				msg, options);
		if (log.isTraceEnabled())
			log.trace("responses: " + retval);
		return retval;
	...
	@Deprecated
	public Object callRemoteMethod(Address dest, MethodCall method_call,
			int mode, long timeout) throws Throwable {
		return callRemoteMethod(dest, method_call, mode, timeout, false);
	}

	@Deprecated
	public Object callRemoteMethod(Address dest, MethodCall method_call,
			int mode, long timeout, boolean oob) throws Throwable {
	...
	@Deprecated
	public Object callRemoteMethod(Address dest, MethodCall call)
			throws Throwable {
		return callRemoteMethod(dest, call, new RequestOptions());
	}

	public Object callRemoteMethod(Address dest, MethodCall call,
			RequestOptions options) throws Throwable {
		if (log.isTraceEnabled())
			log.trace("dest=" + dest + ", method_call=" + call + ", options="
					+ options);

		Object buf = req_marshaller != null ? req_marshaller
				.objectToBuffer(call) : Util.objectToByteBuffer(call);
		Message msg = new Message(dest, null, null);
		if(call.getCallContext() != null && 
				call.getCallContext().getHeaders() != null 
				&& call.getCallContext().getHeaders().size() > 0)
		{
			msg.setBbossHeaders(call.getCallContext().getHeaders());
		}
		// UPDATE ENCRYPT request MESSAGE BODY
		boolean encrypt = SecurityContext.getSecurityManager().enableEncrypt();
		msg.setEncrypt(encrypt);
		if (buf instanceof Buffer) {
			// UPDATE ENCRYPT MESSAGE BODY
			byte[] temp = RequestCorrelator.encrypt((Buffer) buf);
			msg.setBuffer(temp);
		} else {
			// UPDATE ENCRYPT MESSAGE BODY
			byte[] temp = RequestCorrelator.encrypt((byte[]) buf);
			msg.setBuffer(temp);
		}
//		if (buf instanceof Buffer)
//			msg.setBuffer((Buffer) buf);
...
//			msg.setBuffer((byte[]) buf);
	...
	public <T> NotifyingFuture<T> callRemoteMethodWithFuture(Address dest,
			MethodCall method_call, int mode, long timeout, boolean oob)
			throws Throwable {
		RequestOptions options = new RequestOptions(mode, timeout, false, null);
		if (oob)
			options.setFlags(Message.OOB);
	...
	@Deprecated
	public <T> NotifyingFuture<T> callRemoteMethodWithFuture(Address dest,
			MethodCall call) throws Throwable {
		return callRemoteMethodWithFuture(dest, call, new RequestOptions());
	}

	public <T> NotifyingFuture<T> callRemoteMethodWithFuture(Address dest,
			MethodCall call, RequestOptions options) throws Throwable {
		if (log.isTraceEnabled())
			log.trace("dest=" + dest + ", method_call=" + call + ", options="
					+ options);
...
//			msg.setBuffer((byte[]) buf);
	 ...
	 * result. Use MethodCall.invoke() to do this. Return result.
	 */
	public Object handle(Message req) {
		...
		if (server_obj == null) {
			if (log.isErrorEnabled())
				log
						.error("no method handler is registered. Discarding request.");
			return null;
		}
		...
		try {
			body = req_marshaller != null ? req_marshaller
					.objectFromByteBuffer(req.getBuffer(), req.getOffset(), req
							.getLength()) : req.getObject();
		} catch (Throwable e) {
			if (log.isErrorEnabled())
		...
		try {
			...
			if (method_call.getMode() == MethodCall.ID) {
				if (method_lookup == null)
					throw new Exception("MethodCall uses ID="
							+ method_call.getId()
							+ ", but method_lookup has not been set");
			...
			//added by biaoping.yin for security,2010-07-25 16.59 start.			 
			if(method_call.getArgs() != null && method_call.getArgs().length > 0 
					&& method_call.getArgs()[0] instanceof ServiceID)
//			if(securityContext != null)
			{
	            ...
	            context.preMethodCall( id, method, types,req.getBbossHeaders());
			}
          //added by biaoping.yin for security,2010-07-25 16.59 end.
			Object value = method_call.invoke(server_obj);
			System.out.println("method_call:"+value);
			return value;
		} catch (Throwable x) {
			return x;
		}
		finally	//added by biaoping.yin for security,2010-07-25 16.59 .
		{
	 ...
	 * 
	 * @return true if the listener was added or false if the listener was
	 *         already in the list.
	 */
	...
	public void channelConnected(Channel channel) {
		...
		synchronized (additionalChannelListeners) {
			for (Iterator i = additionalChannelListeners.iterator(); i
					.hasNext();) {
