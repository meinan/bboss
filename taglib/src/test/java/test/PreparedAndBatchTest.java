	...
	public static void updateTest() throws SQLException
	{
		for(int i = 0 ; i < 60 ; i ++)
		{
			...
			try
			{
				...
				db.preparedUpdate("bspf","update TD_REG_BANK_ACC_bak set create_acc_time=?,starttime=?,endtime=? where id=951");
...
//				db.addBatch(sql);
	...
	public static void inserttest() throws SQLException
	{
		for(int i = 0 ; i < 60 ; i ++)
		{
			...
			try
			{
				db.preparedInsert("bspf","insert into td_reg_bank_acc_bak (id,create_acc_time,starttime,endtime) values(?,?,?,?)");
				Date today = new Date(new java.util.Date().getTime());
				db.setInt(1, 1000 + i);
...
//	    		tm.begin();
	...
	public static void updateBoubleBlobFile()
	{
			...
			try {
				...
				{
			    	db.preparedUpdate("update TD_REG_BANK_ACC_bak set blob1 = ? where id=?");
			    	db.setBlob(1, new File("D:\\workspace\\cms20080416\\creatorcms\\WEB-INF\\lib\\frameworkset-pool.jar"), "blob1");
//			    	db.setClob(2, "bbb2", "clob2");
//			    	db.setNull(1, java.sql.Types.CLOB);
//			    	db.setNull(2, java.sql.Types.CLOB);
...
//				{
		    	...
		    	db.setBlob(1, new File("D:\\workspace\\cms\\creatorcms\\WEB-INF\\lib\\frameworkset-pool.jar"), "blob1");
//			    	db.setClob(2, "bbb2", "clob2");
//			    	db.setNull(1, java.sql.Types.CLOB);
//			    	db.setNull(2, java.sql.Types.CLOB);
...
//	    		tm.begin();
	...
	public static void insertBlobFileWithOutTableinfo()
	{
...
//			DBUtil dbUtil = new DBUtil();
	    	...
	    	PreparedDBUtil db = new PreparedDBUtil();
			try {
//				dbUtil.executeSelect("select id from testprepared");
...
//	    		for(int i = 0; i < dbUtil.size();i ++)
//				{
		    	db.preparedInsert("insert into testprepared(blob1) values(?)");
		    	db.setBlob(1, new File("D:\\workspace\\cms\\creatorcms\\WEB-INF\\lib\\frameworkset-pool.jar"), "blob1");
//			    	db.setClob(2, "bbb2", "clob2");
//			    	db.setNull(1, java.sql.Types.CLOB);
//			    	db.setNull(2, java.sql.Types.CLOB);
	...
	public static void selectUser() throws SQLException
	{
		for(int i = 0 ; i < 60 ; i ++)
		{
			...
			try
			{
				...
				db.preparedSelect("select user_name from td_sm_user where user_name=?");
				db.setString(1, "admin");
				if(true && i < 50)
				{
					throw new Exception("e");
				}
				db.executePrepared();
//				db.executeSelect("select create_acc_time,starttime,endtime from TD_REG_BANK_ACC_bak where id=951");
				System.out.println("userName = "+db.getString(0, 0));
				//System.out.println("starttime="+db.getDate(0, 1));
				//System.out.println("endtime="+db.getDate(0, 2));
			}
			catch(Exception e)
...
//		
		insertBlobFileWithOutTableinfo();
		//insertBlobFile();
